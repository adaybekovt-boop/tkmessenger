...
function handleIncomingCall(call) {
    ...
    callerNameDisplay.textContent = `${call.peer} вызывает...`;
...
acceptCallBtn.onclick = () => {
    ...
    navigator.mediaDevices.getUserMedia({ audio: true, video: true }).catch(err => {
        ...
        if(stream) {
            ...
            alert('Нет доступа к камере или микрофону. Звонок принят без видео.');
        }
...
async function startCall(videoOn, audioOn, screenStream = null) {
    if (!currentChatFriend) return;
    const conn = activeConnections[currentChatFriend];
    if (!conn || !conn.open) return alert('Пользователь должен быть "В сети" для звонка!');
    ...
    try {
        ...
        return alert('Действие отклонено браузером. Возможно нет прав на веб-камеру.');
    }
