...
function showToast(message, type = 'info') {
    const container = document.getElementById('toast-container') || (function() {
        ...
        c.style.cssText = 'position:fixed;top:20px;left:50%;transform:translateX(-50%);z-index:9999;display:flex;flex-direction:column;gap:10px;pointer-events:none;';
...
(async function bootstrapApp() {
    ...
    showToast('Ошибка запуска приложения. Перезагрузите страницу.', 'error');
});
...
function startOrbit(nick) {
    ...
    peer.on('error', (err) => {
        if(err.type === 'unavailable-id') {
            showToast('Этот позывной уже используется в данный момент! Выбери другой.');
            localStorage.removeItem('orbit_nickname');
            location.reload();
        } else if (err.type === 'peer-unavailable') {
            if (pendingFriendAdd) {
                showToast(`Пользователь "${pendingFriendAdd}" не найден или не в сети! (Сначала нужно войти в сеть)`);
...
async function renderFriends() {
    ...
    for (const f of friends) {
        ...
        avatarLetter.textContent = f.name.substring(0, 2).toUpperCase();

        const statusDot = document.createElement('div');
        statusDot.className = isOnline ? 'friend-status online' : 'friend-status';
        avatar.appendChild(avatarLetter);
        avatar.appendChild(statusDot);

        const infoCol = document.createElement('div');
        infoCol.className = 'friend-info-col';
        ...
        const friendTime = document.createElement('span');
        friendTime.className = 'friend-time';
        friendTime.textContent = timeStr;

        previewRow.appendChild(fPreview);
        previewRow.appendChild(friendTime);

        const trustLine = document.createElement('span');
        trustLine.className = `friend-preview-text ${trustData.className}`;
        trustLine.textContent = trustData.text;

        infoCol.appendChild(fName);
        infoCol.appendChild(previewRow);
        infoCol.appendChild(trustLine);
...
async function startCall(videoOn, audioOn, screenStream = null) {
    if (!currentChatFriend) return;
    if (isOffline) return showToast('Нет сети. Звонок недоступен.');
    const conn = activeConnections[currentChatFriend];
    if (!conn || !conn.open) return showToast(t.callingError);
    ...
    activeCall.on('error', (err) => { showToast('Связь прервалась.'); closeCallUI(); });
    activeCall.on('close', closeCallUI);
}
