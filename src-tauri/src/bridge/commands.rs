...
/// Validate a user-supplied file path before handing it to backend services.
///
/// The frontend is same-origin-trusted but CSP + XSS still matter — a
/// compromised renderer could try to exfiltrate arbitrary files through the
/// file-offer path. We canonicalize (which resolves `..` and symlinks),
/// require the target to exist and be a regular file, and cap the size.
fn validate_user_file_path(raw: &str) -> Result<PathBuf, String> {
    ...
    if raw.contains('\0') {
        return Err("file_path contains NUL byte".into());
    }
...
pub async fn offer_file(
    ...
    let target = parse_peer_id(&peer_id)?;
    let safe_path = validate_user_file_path(&file_path)?;
    state
        .drop
        .offer_file(target, &safe_path)
        .await
        .map_err(err_string)
}
...
/// Check whether the Tauri backend already has a saved identity.
///
/// The React frontend calls this on startup. If `false`, it should
/// read the old keys from IndexedDB and call `import_legacy_identity`
/// before doing anything else.
#[tauri::command]
pub async fn check_legacy_migration(state: State<'_, AppState>) -> Result<bool, String> {
    let db = state.db.clone();
    let existing = tokio::task::spawn_blocking(move || db.load_identity())
        .await
        .map_err(err_string)?
        .map_err(err_string)?;
    // true = identity exists, no migration needed
    // false = no identity yet, frontend should push old keys
    Ok(existing.is_some())
}

/// Import a legacy identity from the old IndexedDB / browser storage.
///
/// The React frontend reads the old keys from IndexedDB on first
/// Tauri launch and sends them here. We only import if there is NO
/// existing identity — this prevents accidental overwrites.
///
/// Expected input (all base64):
///   - signing_key_b64:   Ed25519 private key (32 bytes → b64)
///   - verifying_key_b64: Ed25519 public key  (32 bytes → b64)
///   - display_name:      old profile name
///   - bio:               old profile bio
///   - avatar_b64:        old avatar (optional)
///
/// The peer_id will be derived from the libp2p node identity, NOT
/// from the Ed25519 verifying key, since libp2p uses its own keypair.
#[tauri::command]
pub async fn import_legacy_identity(
    state: State<'_, AppState>,
    ...
    // 1) Only import if no identity exists yet.
    let db = state.db.clone();
    let existing = tokio::task::spawn_blocking(move || db.load_identity())
        .await
        .map_err(err_string)?
    ...
    if existing.is_some() {
        info!("import_legacy_identity: identity already exists — skipping import");
        return Ok(false);
    }

    // 2) Validate the keys are decodable.
    let sk_bytes = BASE64.decode(&signing_key_b64).map_err(err_string)?;
    let vk_bytes = BASE64.decode(&verifying_key_b64).map_err(err_string)?;
    ...
    info!("Legacy identity imported successfully from IndexedDB");

    // 4) Mark migration as done in KV so we don't prompt again.
    let db = state.db.clone();
    tokio::task::spawn_blocking(move || db.kv_set("legacy_migration_done", "true"))
        .await
        .map_err(err_string)?
        .map_err(err_string)?;
