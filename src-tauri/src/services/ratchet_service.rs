...
/// ## Per-peer locking
///
/// `encrypt` and `decrypt` do a read-modify-write on the persisted ratchet
/// state (load → step → save). Without a lock, two concurrent encrypts for
/// the same peer would both load the same state, both derive message keys
/// from the same chain key, and race on save — producing two ciphertexts
/// with identical `Ns` that the peer cannot disambiguate, and silently
/// losing a ratchet step. The per-peer async mutex below serializes these
/// operations *for the same peer* while still allowing full parallelism
/// across peers.
pub struct RatchetService {
    db: Database,
    /// Key used to encrypt ratchet state at rest.
    vault_key: [u8; 32],
    /// peer_id → lock. Outer ParkingMutex guards the map itself (short
    /// critical section for lookup/insert); the inner AsyncMutex is held
    /// across await points during load/encrypt/save.
    peer_locks: ParkingMutex<HashMap<String, Arc<AsyncMutex<()>>>>,
}
...
impl RatchetService {
    pub fn new(db: Database, vault_key: [u8; 32]) -> Self {
        Self {
            db,
            vault_key,
            peer_locks: ParkingMutex::new(HashMap::new()),
        }
    }

    /// Get (or create) the per-peer lock. The returned Arc can be locked
    /// across .await points without holding the outer map mutex.
    fn peer_lock(&self, peer_id: &str) -> Arc<AsyncMutex<()>> {
        let mut map = self.peer_locks.lock();
    ...
    pub async fn delete_session(&self, peer_id: &str) -> Result<()> {
        // Serialize with any in-flight encrypt/decrypt so we don't delete
        // a session out from under a task that is mid-ratchet-step.
        let lock = self.peer_lock(peer_id);
        let _guard = lock.lock().await;

        let id = peer_id.to_string();
        let db = self.db.clone();
        let res = tokio::task::spawn_blocking(move || db.delete_ratchet_session(&id))
            .await
            .map_err(|e| OrbitsError::Internal(format!("spawn_blocking: {e}")))?;

        // Drop the peer lock entry now that the session is gone. Any future
        // operation for this peer will create a fresh lock.
        self.peer_locks.lock().remove(peer_id);
        res
    }
    ...
    ///
    /// Acquires a per-peer async mutex for the whole load→step→save cycle
    /// so two concurrent encrypts for the same peer cannot derive duplicate
    /// message keys from the same chain state.
    ...
    ///
    /// Same per-peer serialization as `encrypt` — decryption also mutates
    /// the ratchet (receive chain advance + skipped-key bookkeeping), so
    /// concurrent decrypts for the same peer must not interleave.
