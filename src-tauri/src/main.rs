...
async fn start_backend(
    ...
    // ── Network ─────────────────────────────────────────────────
    let listen_addrs = net::default_listen_addrs();
    let (net_handle, event_rx, _net_join) = net::start(None, listen_addrs).await?;
    ...
    // ── Bootstrap Kademlia ──────────────────────────────────────
    // Give the swarm a moment to start listening before bootstrapping.
    ...
    if let Err(e) = app
        ...
        .await
    {
        tracing::warn!(?e, "Initial Kademlia bootstrap failed");
    }
