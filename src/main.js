...
function setupLazyImageObserver() {
  ...
  root.addEventListener('scroll', () => {
    ...
    scrollDebounceTimer = setTimeout(() => {
      // handle scroll-end logic (e.g., mark messages as read)
    }, 150);
  }, { passive: true });
...
function wireAppearanceControls() {
  ...
  if (openCustomizerBtn && customizerModal) {
    openCustomizerBtn.addEventListener('click', () => {
      ...
      document.getElementById('reduce-animations-toggle').checked = appSettings.reduceAnimations || false;
  ...
  if (applyCustomThemeBtn) {
    applyCustomThemeBtn.addEventListener('click', () => {
      ...
      appSettings.reduceAnimations = document.getElementById('reduce-animations-toggle').checked;
      applyAppearanceSettings();
      saveSettings();
...
let cinemaVideoEl = document.getElementById('cinema-video');
...
document.getElementById('close-cinema-btn').addEventListener('click', () => {
  toggleCinemaMode(null);
});

document.getElementById('cinema-file-input').addEventListener('change', (e) => {
  const file = e.target.files[0];
  if (!file) return;
...
['play', 'pause', 'seeked'].forEach(eventName => {
  cinemaVideoEl.addEventListener(eventName, () => {
    if (isCinemaRemoteAction) {
      isCinemaRemoteAction = false; // Reset lock
      return;
    }
...
cinemaVideoEl.addEventListener('play', () => {
  if (cinemaSyncInterval) clearInterval(cinemaSyncInterval);
...
cinemaVideoEl.addEventListener('pause', () => {
  if (cinemaSyncInterval) clearInterval(cinemaSyncInterval);
});
...
function triggerLocalNudge() {
  ...
  document.getElementById('file-btn').addEventListener('click', () => {
    document.getElementById('orbits-drop-input').click();
  });
  
  document.getElementById('orbits-drop-input').addEventListener('change', (e) => {
    const file = e.target.files[0];
  ...
  document.getElementById('drop-send-btn').addEventListener('click', () => {
    const file = window._pendingDropFile;
    if (!file) return;
    
    const qualitySetting = document.querySelector('input[name="drop_quality"]:checked').value;
    
    document.getElementById('drop-quality-modal').style.display = 'none';
  ...
  if (saved) {
    try {
      ...
      if (nick && hash) {
        ...
        await cryptoDerive(hash, nick, 'orbits_salt'); // Use hash as password for AES in autologin
  ...
  if (msg.type === 'text') {
    ...
    if (msg.dropPercent !== undefined && msg.dropPercent < 100) {
      ...
      contentHtml = `<div class="msg-file">
        ...
        <a href="${msg.url}" download="${escapeAttr(msg.name)}" style="color: inherit; text-decoration: none;">${escapeHtml(msg.name || 'File')}</a>
      </div>`;
    }
  }
...
function openSettingsPanel() {
  ...
  setTimeout(() => panel.removeAttribute('aria-hidden'), 10);
