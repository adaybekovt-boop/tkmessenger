...
export class CallManager extends MicroEmitter {
  ...
  async end() {
    // Transition to ENDING before we release resources so the UI can show
    // a "hanging up…" frame if it wants.
    const wasStatus = this._state.status;
    if (wasStatus === CallStatus.IDLE) return;
    this.ringtone.stop();
    this._transitionTo(CallStatus.ENDING, { screenSharing: false });
    this._teardownChannel();
    this.pool.stopAll();
    this._transitionTo(CallStatus.IDLE, {
      remoteId: '',
