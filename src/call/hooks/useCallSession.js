...
export function useCallSession({ peerRef, peerIdRef, blockedPeersRef } = {}) {
  ...
  useEffect(() => {
    ...
    // Track error-clear timer in the effect closure so the cleanup can cancel
    // a pending clear and avoid `setState` after unmount.
    ...
    const offError = mgr.on('error', (err) => {
      ...
      const msg = err?.message || String(err || 'Ошибка звонка');
      setLastError(msg);
  ...
  return useMemo(() => ({
    ...
    state, lastError, dismissError, startCall, acceptCall, rejectCall, endCall, toggleAudio, toggleVideo,
