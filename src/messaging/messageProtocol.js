...
import { safeAvatarDataUrl } from '../utils/common.js';
import { buildLocalBundle, serializeBundle } from '../core/prekeyBundle.js';
import { acceptIncomingBundle } from '../core/bundleCache.js';
...
export function dispatchReliablePlaintext(data, conn, remoteId, ctx) {
  ...
  if (data.type === 'profile_res') {
    ...
    // Remote avatars are untrusted input — validate MIME and size strictly.
    // A peer could send `data:image/svg+xml,...` with embedded scripts / remote
    // resource fetches, or a 10MB data URL to DoS localStorage/IDB.
    const safeAvatar = safeAvatarDataUrl(p.avatarDataUrl);
    if (safeAvatar) {
      try { void ctx.saveAvatar(remoteId, safeAvatar); } catch (_) {}
    } else if (p.avatarDataUrl === null || p.avatarDataUrl === '' || p.avatarDataUrl === undefined) {
      // Peer explicitly removed their avatar — drop the stale copy from IDB so
      // the next reload doesn't resurrect it through `getAvatar`-based hydrate.
      try { void ctx.deleteAvatar?.(remoteId); } catch (_) {}
    }
    const safeDisplayName = String(p.displayName || remoteId).slice(0, 64);
    // Mirror the displayName onto the peers contact list so it survives a
    // reload even if the `orbits_profiles_v1` localStorage cache is wiped.
    try { ctx.upsertPeer?.(remoteId, { displayName: safeDisplayName, lastSeenAt: Date.now() }); } catch (_) {}
  ...
  // ─── X3DH prekey bundle exchange ──────────────────────────────
  //
  // Peers push/pull each other's prekey bundles over the already-authenticated
  // reliable channel so a future X3DH session can start without a fresh
  // wireHello round-trip. Bundle exchange piggy-backs on the ratchet: both
  // ends already know who they're talking to via the identity signature on
  // the wireHello, so there is no need for bundles to carry a second
  // transport-level authenticator.
  if (data.type === 'bundle_req') {
    const nonce = typeof data.nonce === 'number' ? data.nonce : Date.now();
    const selfPeerId = ctx.peerIdRef?.current;
    if (!selfPeerId) return true;
    void (async () => {
      try {
        const bundle = await buildLocalBundle({ peerId: selfPeerId });
        sendReply({ type: 'bundle_res', nonce, bundle: serializeBundle(bundle) });
      } catch (err) {
        try { console.warn('[bundle] build failed', err?.message || err); } catch (_) {}
      }
    })();
    return true;
  }

  if (data.type === 'bundle_res') {
    const wire = data.bundle;
    if (!wire || typeof wire !== 'object') return true;
    void (async () => {
      const result = await acceptIncomingBundle({ senderPeerId: remoteId, wire });
      if (result.ok) {
        try { ctx.onBundleAccepted?.(remoteId, result); } catch (_) {}
      } else {
        try { ctx.onBundleRejected?.(remoteId, result); } catch (_) {}
        try { console.warn('[bundle] rejected', remoteId, result.reason); } catch (_) {}
      }
    })();
    return true;
  }
  ...
  // Game channel — mini-games piggy-back on the encrypted reliable channel
  // without polluting the chat history. The payload travels inside
  // `data.payload`; dispatch hands it off to subscribers registered via
  // usePeer(). No ack, no persistence.
  if (data.type === 'game') {
    try { ctx.onGameMessage?.(remoteId, data.payload); } catch (_) {}
    return true;
  }
  ...
  const attachmentMeta = data.attachment && typeof data.attachment === 'object' ? data.attachment : null;
  ...
  void (async () => {
    ...
    let voiceRef = null;
    const transcript = typeof voiceMeta?.transcript === 'string' ? voiceMeta.transcript.slice(0, 2000) : '';
    ...
    // Attachment (image / video / file): decode base64 → blob, persist in
    // file_blobs store under the message id, then strip the b64 from the
    // UI-side metadata (the VoicePlayer / AttachmentBubble will lazy-load
    // the blob on demand via getFileBlob).
    let attachmentRef = null;
    if (attachmentMeta) {
      const name = String(attachmentMeta.name || 'file').slice(0, 200);
      const mime = String(attachmentMeta.mime || 'application/octet-stream');
      const kind = String(attachmentMeta.kind || 'file');
      const size = Number(attachmentMeta.size) || 0;
      const width = Number(attachmentMeta.width) || 0;
      const height = Number(attachmentMeta.height) || 0;
      const duration = Number(attachmentMeta.duration) || 0;
      const thumb = typeof attachmentMeta.thumb === 'string' ? attachmentMeta.thumb : null;
      if (typeof attachmentMeta.b64 === 'string') {
        try {
          ...
          // Fall back to metadata-only: preview shows, but the open/save action is disabled.
          attachmentRef = { name, size, mime, kind, thumb, width, height, duration, missing: true };
        }
      } else {
        attachmentRef = { name, size, mime, kind, thumb, width, height, duration, missing: true };
      }
    ...
    const preview = msgType === 'sticker'
      ...
      : msgType === 'voice'
        ? '🎤 Голосовое'
        : msgType === 'file'
          ? (attachmentRef?.kind === 'image' ? '🖼 Фото' : attachmentRef?.kind === 'video' ? '🎬 Видео' : `📎 ${attachmentRef?.name || 'Файл'}`)
          : text;
