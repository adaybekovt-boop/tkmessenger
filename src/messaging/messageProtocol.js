...
import { isWireCiphertext } from '../core/doubleRatchet.js';
import {
  decryptInbound as wireDecryptInbound,
  acceptHello as wireAcceptHello
...
export async function dispatchReliableInbound(data, conn, remoteId, ctx) {
  ...
  if (data && typeof data === 'object' && (data.type === 'wireHello' || data.type === 'wireRekey')) {
    try {
      const { reply } = await wireAcceptHello(remoteId, ctx.peerIdRef.current, data);
      if (reply) {
  ...
  if (isWireCiphertext(data)) {
    let plaintext;
    try {
      plaintext = await wireDecryptInbound(remoteId, data);
    } catch (err) {
