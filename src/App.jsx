...
import { Download, MessageSquare, Radar, Send, Settings2, X } from 'lucide-react';
import OrbitsLogo from './components/OrbitsLogo.jsx';
import Onboarding from './components/Onboarding.jsx';
import Chats from './pages/Chats.jsx';
import RadarView from './pages/Radar.jsx';
import DropView from './pages/Drop.jsx';
...
function TabButton({ active, icon: Icon, onClick }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={cx(
        'flex flex-1 items-center justify-center rounded-xl py-2 transition-colors duration-200 active:scale-95',
        active
  ...
  return (
    <PeerProvider>
      ...
      >
        ...
        >
          <TabButton active={tab === 'chats'} icon={MessageSquare} onClick={() => { hapticTap(); setTab('chats'); }} />
          <TabButton active={tab === 'radar'} icon={Radar} onClick={() => { hapticTap(); setTab('radar'); }} />
          <TabButton active={tab === 'drop'} icon={Send} onClick={() => { hapticTap(); setTab('drop'); }} />
          <TabButton active={tab === 'settings'} icon={Settings2} onClick={() => { hapticTap(); setTab('settings'); }} />
        </nav>
      </div>
    </PeerProvider>
  );
