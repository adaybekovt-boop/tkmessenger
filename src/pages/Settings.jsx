...
function Toggle({ checked, onChange, label }) {
  return (
    <button
      ...
      // py-2.5 + h-6 thumb → overall height ≥44px to meet iOS HIG
      // tap-target. touch-action: manipulation kills the 300ms tap
      // delay that old Android WebViews still impose.
      className="inline-flex items-center gap-3 rounded-2xl bg-[rgb(var(--orb-bg-rgb))]/45 px-3 py-2.5 text-xs text-[rgb(var(--orb-text-rgb))] ring-1 ring-[rgb(var(--orb-border-rgb))] transition-all duration-300 ease-in-out active:scale-95"
      style={{ touchAction: 'manipulation' }}
      aria-pressed={checked}
    >
      <span className={cx('h-6 w-10 rounded-full ring-1 ring-[rgb(var(--orb-border-rgb))] transition-all duration-300 ease-in-out', checked ? 'bg-[rgb(var(--orb-accent-rgb))]/60' : 'bg-[rgb(var(--orb-surface-rgb))]/60')}>
        <span className={cx('block h-5 w-5 translate-y-0.5 rounded-full bg-white transition-all duration-300 ease-in-out', checked ? 'translate-x-[1.125rem]' : 'translate-x-0.5')} />
      </span>
...
function ActionCard({ icon: Icon, title, subtitle, onClick, tone }) {
  ...
  return (
    <button
      ...
      // 11×11 tiles on a phone screen. touch-action: manipulation kills
      // the 300ms tap delay on old Android WebViews.
...
//
// The header uses `position: sticky; top: 0` so when the list scrolls
// up it stays pinned to the top while the scroll-driven transforms
// shrink it into a compact bar. Without sticky it would just scroll
// away — which is what the first pass did and is NOT Telegram-like.
//
// Props:
//   user       — { displayName, username, avatarDataUrl }
//   scrollEl   — the DOM element of the scrollable container (not a ref).
//                We take the element directly so there's no "ref is null
//                on first render → useScroll falls back to window" bug.
//                The parent forces a re-render after attach via callback
                ...
                <Section icon={UserRound} title="Профиль" subtitle="Имя, аватар и описание">
                  ...
                  <Row label="Отображаемое имя">
                    <input
                      ...
                      onChange={(e) => {
                        setProfileError('');
                        // Strip leading whitespace so a user can't stuff
                        // 32 spaces and get a blank-looking row. Trailing
                        // whitespace is kept until submit (`.trim()` on
                        // save) so typing mid-word "Дмитр " doesn't
                        // eat the space mid-flow.
                        setProfileDisplayName(e.target.value.replace(/^\s+/, ''));
                      }}
                      maxLength={32}
                      // Native mobile keyboard hints: nickname-context
                      // autocomplete, no autocorrect / spellcheck (names
                      // aren't dictionary words), enter-key = Done so the
                      // software keyboard dismisses instead of newline.
                  ...
                  <Row label="Описание">
                    <textarea
                      ...
                      maxLength={200}
                      rows={3}
                      // Sentences: capitalise after '.', keep spellcheck
                      // on (free-form prose), enterKeyHint stays as
                      // newline because a bio is multi-line.
