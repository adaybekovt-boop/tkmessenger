...
import { Activity, Bell, ChevronLeft, ClipboardCopy, Cpu, Lock, LogOut, MessageSquare, Mic2, Moon, Palette, PlugZap, RefreshCw, Shield, SlidersHorizontal, Sparkles, Trash2, UserRound, Zap } from 'lucide-react';
...
export default function Settings({ swState, onCheckUpdate, onReloadNow, theme, setTheme, powerSaver, setPowerSaver, notifPermission, requestNotifPermission }) {
  ...
  // ── Security: Wipe-on-Close & Duress Password ──
            ...
            >
              ...
              {screen === 'profile' ? (
                <Section icon={Shield} title="Профиль и сеть" subtitle="Твой уникальный ID для друзей">
                  ...
                  <Row label="Сбросить ID">
                    <button
                      type="button"
                      onClick={() => {
                        const ok = window.confirm('Сбросить Peer ID? Друзьям нужно будет добавить новый ID.');
                        if (!ok) return;
                        peer.resetIdentity?.();
                      }}
                      className="inline-flex items-center gap-2 rounded-2xl bg-[rgb(var(--orb-danger-rgb))]/10 px-3 py-2 text-xs font-semibold text-[rgb(var(--orb-danger-rgb))] ring-1 ring-[rgb(var(--orb-danger-rgb))]/20 transition-all duration-300 ease-in-out active:scale-95"
                    >
                ...
                <Section icon={Lock} title="Безопасность" subtitle="Шифрование и защита данных">
                  <div className="grid gap-3">
                    <div className="rounded-3xl bg-[rgb(var(--orb-bg-rgb))]/35 p-4 ring-1 ring-[rgb(var(--orb-border-rgb))]">
                      ...
                      <div className="mt-3 grid gap-2">
                        <div className="flex items-center justify-between gap-3 py-1">
                          ...
                          <span className="rounded-full bg-green-500/15 px-2 py-0.5 text-[10px] font-semibold text-green-400 ring-1 ring-green-500/25">ACTIVE</span>
                        </div>
                        <div className="flex items-center justify-between gap-3 py-1">
                          <div>
                    ...
                    <div className="rounded-3xl bg-[rgb(var(--orb-bg-rgb))]/35 p-4 ring-1 ring-[rgb(var(--orb-border-rgb))]">
                      ...
                      <div className="mt-2">
                        <span className="rounded-full bg-green-500/15 px-2 py-0.5 text-[10px] font-semibold text-green-400 ring-1 ring-green-500/25">ACTIVE — 5 мин</span>
                      </div>
                    </div>
                  </div>
                </Section>
                    ...
                    <div className="rounded-3xl bg-[rgb(var(--orb-bg-rgb))]/35 p-4 ring-1 ring-[rgb(var(--orb-border-rgb))]">
                      <div className="flex items-center justify-between gap-3">
                        <div>
                          ...
                          <div className="mt-0.5 text-xs text-[rgb(var(--orb-muted-rgb))]">Удалит все сообщения со статусом «pending»</div>
                        </div>
                        <button
                          type="button"
                          onClick={async () => {
                  ...
                  <div className="grid gap-3">
                    ...
                    <div className="rounded-2xl bg-[rgb(var(--orb-bg-rgb))]/35 p-3 ring-1 ring-[rgb(var(--orb-border-rgb))]">
                      ...
                      <div className="mt-2 grid gap-1 text-xs text-[rgb(var(--orb-muted-rgb))]">
                        ...
                        <div className="font-mono">{device.userAgent}</div>
                      </div>
                    </div>
