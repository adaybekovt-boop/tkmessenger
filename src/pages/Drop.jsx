      ...
      <AnimatePresence mode="wait">
        ...
        {status === 'beacon' && (
          ...
          >
            ...
            >
              {(() => {
                const LeadIcon = selectedFiles.length === 1
                  ? getFileIcon({ mime: selectedFiles[0].type, name: selectedFiles[0].name })
                  : FileUp;
                return <LeadIcon className="h-5 w-5 text-[rgb(var(--orb-accent-rgb))]" />;
              })()}
              <div className="min-w-0 flex-1">
        ...
        {status === 'awaiting-consent' && (
          ...
          >
            ...
            {session?.files?.length > 0 && (
              <div className="w-full max-w-xs space-y-1.5 rounded-xl bg-[rgb(var(--orb-surface-rgb))]/40 p-3">
                {session.files.slice(0, 5).map((f, i) => {
                  const Icon = getFileIcon({ mime: f.mime, name: f.name });
                  return (
                    <div key={i} className="flex items-center gap-2 text-xs">
                      <Icon className="h-4 w-4 shrink-0 text-[rgb(var(--orb-accent-rgb))]" />
                      <span className="min-w-0 flex-1 truncate text-[rgb(var(--orb-text-rgb))]">{f.name}</span>
                      <span className="shrink-0 text-[rgb(var(--orb-muted-rgb))]">{formatSize(f.size)}</span>
                    </div>
                  );
                })}
