...
function Avatar({ profile, fallback, online }) {
  ...
  // Guard against broken / truncated dataURL — otherwise the browser
  // shows its own "broken image" glyph inside our circle. Reset whenever
  // the image src changes (e.g. peer switched profile) via key={img}.
...
function PeerRow({ peer, active, meta, onClick }) {
  ...
  return (
    <button
      ...
      // touch-action: manipulation — kills the 300ms tap delay that old
      // Android WebViews still apply on non-double-tap-zoom elements.
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-0.5 flex items-center justify-between gap-2">
            ...
            {unread > 0 ? (
              // Cap the badge text at "99+" so a chat with many unread
              // messages doesn't stretch the badge wide enough to push
              // the timestamp row off-screen on narrow phones.
...
function ReplyPreviewInline({ replyTo, mine }) {
  // Reach into the peer context so we can resolve the reply-author's
  // nickname from the profiles cache. Previously this only had access
  // to `replyTo.from` (raw id) and fell back to the "Контакт •XXXX"
  // placeholder even for peers whose profile was already loaded —
  // confusing UX in threaded conversations.
  const peerCtx = usePeerContext();
  ...
  const replyProfile = replyTo.from ? peerCtx?.profilesByPeer?.[replyTo.from] : null;
  const replyPeer = replyTo.from ? peerCtx?.peers?.find((p) => p.id === replyTo.from) : null;
  const authorName =
    replyTo.fromName ||
    (replyTo.from
      ? peerDisplayName({ profile: replyProfile, peer: replyPeer, id: replyTo.from })
      : 'Сообщение');
  return (
    <div
      className={cx(
    ...
    >
      <div className="font-semibold text-[rgb(var(--orb-accent-rgb))]">{authorName}</div>
  ...
  return (
    <div className="orb-page-bg flex h-full w-full overflow-hidden">
      ...
      <section
        ...
        // touch-action: pan-y pinch-zoom — vertical scroll stays natural
        // and horizontal browser-swipe-back is suppressed, but pinch
        // gestures still work on nested <img>/<video> attachments. Plain
        // `pan-y` (без pinch-zoom) убивало зум фото пальцами — а это
        // базовое нативное ожидание в мессенджере.
                  ...
                  <div className="rounded-2xl bg-[rgb(var(--orb-surface-rgb))]/20 p-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        ...
                        {/* Raw ORBITS-id used to be shown here as-is.
                            That's noise in normal use — the nickname is
                            above — but the user still occasionally needs
                            to copy it for cross-device verification. So
                            we show a short "•LAST4" chip that copies the
                            full id to the clipboard on tap. */}
                        <button
                          type="button"
                          onClick={async () => {
                            hapticTap();
                            try { await navigator.clipboard?.writeText(activeId); } catch (_) {}
                          }}
                          title="Скопировать ID"
                          aria-label={`Скопировать ID: ${activeId}`}
                          style={{ touchAction: 'manipulation' }}
                          className="mt-0.5 inline-flex items-center gap-1 rounded-md font-mono text-[11px] text-[rgb(var(--orb-muted-rgb))] transition-opacity duration-150 active:opacity-60"
                        >
                          <ClipboardCopy className="h-3 w-3 opacity-70" />
                          ID •{String(activeId).slice(-4).toUpperCase()}
                        </button>
                      </div>
                    </div>
                    {activeProfile?.bio ? (
