...
import { isRelayOnlyEnabled } from './helpers.js';
...
export function createPeerInstance({ id, host, env }) {
  ...
  // Paranoid mode: only accept relay (TURN) candidates so remote never sees
  // our IP. Silently ignored if no TURN server is configured — `'relay'`
  // without a TURN URL is an unreachable config.
  const rtcConfig = { iceServers };
  if (hasTurn && isRelayOnlyEnabled()) {
