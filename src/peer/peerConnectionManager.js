...
export class PeerConnectionManager {
  ...
  constructor({ desiredPeerId, env, callbacks, isDropInProgressRef }) {
    ...
    // Once the PeerJS server has ACKed our id at least once, transient
    // signal-server / socket-error blips are routine noise — the server
    // auto-recovers and our reconnect loop handles the rest. We track
    // this flag so we can hide those errors from the red banner after
    // the first successful handshake. First-ever connect still surfaces
    // errors (user needs to know if bootstrap is failing).
    this._hasEverConnected = false;
    // Counter of consecutive signal-server errors since the last clean
    // connection. After a threshold we do show the banner even when
    // previously connected, so real extended outages aren't hidden.
    this._signalNoiseStreak = 0;
    // Timer that clears a stale signal-server error from the UI after a
    // quiet period — prevents the banner from sticking forever on a
    // single transient blip.
    this._signalNoiseClearTimer = null;
  ...
  _buildHandlers() {
    ...
    const onOpen = (id) => {
      ...
      self._signalNoiseStreak = 0;
      if (self._signalNoiseClearTimer) {
        clearTimeout(self._signalNoiseClearTimer);
        self._signalNoiseClearTimer = null;
      }
    ...
    const onError = (err) => {
      ...
      // Signal-server hiccups (server-error / socket-error) are routine
      // noise on public PeerJS pools — the server auto-recovers in 5-30s
      // and our reconnect / host-rotation logic below handles the rest.
      // Surface rules:
      //   (a) peer currently open → swallow (listAllPeers blips, etc.);
      //   (b) we've connected before, blip count < threshold → swallow,
      //       keep status as 'connecting' so the user sees a gentle
      //       "подключение…" instead of a scary red banner; auto-clear
      //       any residual error after ~10s of quiet;
      //   (c) first-ever connection failing, or streak > threshold →
      //       show the error text (real extended outage — user needs
      //       to know the signaling layer is down).
      const isSignalNoise = err?.type === 'server-error' || err?.type === 'socket-error';
      const transient = isSignalNoise || err?.type === 'network' || err?.type === 'peer-unavailable';

      if (transient && self.peer && self.peer.open) {
        // (a) peer is functional — user-invisible.
        self.cb.onError?.(err);
      } else if (isSignalNoise && self._hasEverConnected && self._signalNoiseStreak < 5) {
        // (b) soft-retry window. Downgrade to "connecting…" status.
        self._signalNoiseStreak += 1;
        self.cb.setStatus?.('connecting');
        self.cb.setError?.(null);
        if (self._signalNoiseClearTimer) clearTimeout(self._signalNoiseClearTimer);
        self._signalNoiseClearTimer = setTimeout(() => {
          self._signalNoiseClearTimer = null;
          // If we reconnected in the meantime, the streak was already
          // reset in onOpen — nothing to do here.
          self._signalNoiseStreak = 0;
        }, 10000);
        self.cb.onError?.(err);
      } else {
        // (c) real error worth showing.
        if (isSignalNoise) self._signalNoiseStreak += 1;
  ...
  stop() {
    ...
    if (this._signalNoiseClearTimer) {
      clearTimeout(this._signalNoiseClearTimer);
      this._signalNoiseClearTimer = null;
    }
    try { this.multiTabLock?.release(); } catch (_) {}
    this.multiTabLock = null;
