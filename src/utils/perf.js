export const isLowEnd = typeof navigator !== 'undefined' && (navigator.deviceMemory || 4) < 2;

if (typeof document !== 'undefined' && isLowEnd) {
...
export function setupPerfOverlayToggle(getters) {
  const toggle = () => {
    if (_overlayActive) {
      _overlayCleanup?.();
      ...
      _overlayCleanup = startDevPerfOverlay(getters, true);
      _overlayActive = true;
    }
  };
 ...
 */
export function startDevPerfOverlay(getters, force = false) {
  const enabled = force || (typeof import.meta !== 'undefined' && import.meta.env && import.meta.env.DEV) || _isLocalhost;
  if (!enabled) return () => {};
