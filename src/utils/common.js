 ...
 *
 * Fallback priority:
 *   1. profile.displayName   — remote-provided nickname (most accurate)
 *   2. profile.username      — legacy / alternate key that some callers
 *                              populate instead of displayName
 *   3. peer.displayName      — locally-set contact label
 *   4. peer.username         — same, alternative key
 *   5. shortPeerLabel(id)    — "Контакт •XXXX" placeholder
 *
 * The returned string is always clipped to 32 chars with a trailing
 * ellipsis so a peer that ships a 500-char "nickname" can't blow up
 * the row layout.
 */
export function peerDisplayName({ profile, peer, id } = {}) {
  ...
  for (const raw of candidates) {
    const s = (raw || '').trim();
    if (s) return s.length > 32 ? `${s.slice(0, 31)}…` : s;
  }
