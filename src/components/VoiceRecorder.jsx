    ...
    (async () => {
      try {
        ...
        interval = setInterval(() => {
          ...
          setTranscript(recRef.current.getTranscript?.() || '');
        }, 200);
      } catch (e) {
    ...
    >
      ...
      <div className="flex flex-1 flex-col gap-0.5 min-w-0">
        <div className="flex items-center gap-2">
          ...
          <span className="font-mono text-xs text-[rgb(var(--orb-text-rgb))]">{formatDuration(elapsed)}</span>
          <Waveform levels={levels} live />
        </div>
        {transcript ? (
          <div className="truncate text-[11px] text-[rgb(var(--orb-muted-rgb))]" title={transcript}>
            {transcript}
          </div>
        ) : null}
      </div>
