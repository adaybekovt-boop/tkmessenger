import React from 'react';
import { reportError, formatReportForClipboard } from '../core/errorReporter.js';

const IS_DEV = typeof import.meta !== 'undefined' && import.meta.env?.DEV === true;

export default class ErrorBoundary extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      hasError: false,
      message: '',
      stack: '',
      report: null,
      copied: false,
    };
  }

  static getDerivedStateFromError(error) {
    return {
      hasError: true,
      message: String(error?.message || error || 'Ошибка'),
      stack: String(error?.stack || ''),
    };
  }

  componentDidCatch(error, info) {
    const report = reportError(error, {
      source: 'react',
      componentStack: info?.componentStack || null,
    });
    this.setState({ report });
  }

  _handleReset = () => {
    this.setState({ hasError: false, message: '', stack: '', report: null, copied: false });
  };

  _handleReload = () => {
    try { window.location.reload(); } catch (_) {}
  };

  _handleCopy = async () => {
    try {
      const text = formatReportForClipboard(this.state.report) || this.state.message;
      await navigator.clipboard.writeText(text);
      this.setState({ copied: true });
      setTimeout(() => {
        if (this.state?.hasError) this.setState({ copied: false });
      }, 2000);
    } catch (_) {
    }
  };

  render() {
    if (!this.state.hasError) return this.props.children;

    return (
      <div
        className="flex h-[100dvh] w-full items-center justify-center bg-[rgb(var(--orb-bg-rgb))] px-4"
        style={{ paddingTop: 'env(safe-area-inset-top)', paddingBottom: 'env(safe-area-inset-bottom)' }}
      >
        <div className="w-full max-w-[520px]">
          <div className="orb-blur rounded-[28px] bg-[rgb(var(--orb-surface-rgb))]/30 p-5 ring-1 ring-[rgb(var(--orb-border-rgb))]">
            <div className="text-sm font-semibold text-[rgb(var(--orb-text-rgb))]">Что-то пошло не так</div>
            <div className="mt-2 text-xs text-[rgb(var(--orb-muted-rgb))] break-words">{this.state.message}</div>

            {IS_DEV && this.state.stack ? (
              <pre className="mt-3 max-h-56 overflow-auto rounded-xl bg-black/40 p-3 text-[10px] leading-relaxed text-[rgb(var(--orb-muted-rgb))]">
                {this.state.stack}
              </pre>
            ) : null}

            <div className="mt-4 flex flex-wrap gap-2">
              <button
                type="button"
                onClick={this._handleReload}
                className="inline-flex h-11 items-center justify-center rounded-2xl bg-[rgb(var(--orb-accent-rgb))] px-4 text-sm font-semibold text-white transition-all duration-300 ease-in-out active:scale-95"
              >
                Перезагрузить
              </button>
              <button
                type="button"
                onClick={this._handleReset}
                className="inline-flex h-11 items-center justify-center rounded-2xl bg-[rgb(var(--orb-bg-rgb))]/35 px-4 text-sm font-semibold text-[rgb(var(--orb-text-rgb))] ring-1 ring-[rgb(var(--orb-border-rgb))] transition-all duration-300 ease-in-out active:scale-95"
              >
                Попробовать снова
              </button>
              <button
                type="button"
                onClick={this._handleCopy}
                className="inline-flex h-11 items-center justify-center rounded-2xl bg-[rgb(var(--orb-bg-rgb))]/35 px-4 text-sm font-semibold text-[rgb(var(--orb-text-rgb))] ring-1 ring-[rgb(var(--orb-border-rgb))] transition-all duration-300 ease-in-out active:scale-95"
              >
                {this.state.copied ? 'Скопировано ✓' : 'Копировать лог'}
              </button>
            </div>
          </div>
        </div>
      </div>
    );
  }
}
