...
// The previous 'gradient' preset used bg-gradient-to-br from accent → accent2.
// User requested flat colors only. Kept the `gradient` id for back-compat with
// existing localStorage (`chatPrefs.myColor === 'gradient'`) but swapped its
// class to a solid accent fill.
const MY_COLOR_PRESETS = [
  { id: 'gradient', label: 'Акцент',   bg: 'bg-[rgb(var(--orb-accent-rgb))]',    ring: 'ring-[rgb(var(--orb-accent-rgb))]/40', text: 'text-white', meta: 'text-white/70' },
  { id: 'accent',   label: 'Мягкий',   bg: 'bg-[rgb(var(--orb-accent-rgb))]/20', ring: 'ring-[rgb(var(--orb-accent-rgb))]/30', text: '', meta: '' },
...
export function getMyBubbleColors(prefs) {
  ...
  // Fallback is flat accent — no gradient.
  return 'bg-[rgb(var(--orb-accent-rgb))] ring-1 ring-[rgb(var(--orb-accent-rgb))]/40 text-white';
}
