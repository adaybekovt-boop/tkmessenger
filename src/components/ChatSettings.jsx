...
const MY_COLOR_PRESETS = [
  { id: 'accent',  label: 'Акцент',   bg: 'bg-[rgb(var(--orb-accent-rgb))]/18', ring: 'ring-[rgb(var(--orb-accent-rgb))]/30' },
...
const PEER_COLOR_PRESETS = [
  { id: 'default', label: 'По умолч.', bg: 'bg-[rgb(var(--orb-surface-rgb))]/70', ring: 'ring-[rgb(var(--orb-border-rgb))]' },
...
export function getMyBubbleColors(prefs) {
  ...
  return 'bg-[rgb(var(--orb-accent-rgb))]/18 ring-1 ring-[rgb(var(--orb-accent-rgb))]/30';
}

export function getPeerBubbleColors(prefs) {
  const preset = PEER_COLOR_PRESETS.find((p) => p.id === prefs?.peerColor);
  if (preset) return `${preset.bg} ring-1 ${preset.ring}`;
  return 'bg-[rgb(var(--orb-surface-rgb))]/70 ring-1 ring-[rgb(var(--orb-border-rgb))]';
}
