...
const CATALOG = {
  // ─── Classic (tokens only, no background layer) ──────────────────────────
  ...
  // ─── Atmospheric (lazy bundle per theme) ────────────────────────────────
  // Registered here for forward-compat; manifests ship in later PRs.
  // Leaving the key live with a missing file would crash — so atmospheric
  // themes are added to this map only when their folder exists on disk.
};
...
export const LEGACY_THEME_MAP = Object.freeze({
  ...
  sakura_zen:  'classic-dark',  // Atmospheric Sakura ships in its own PR
