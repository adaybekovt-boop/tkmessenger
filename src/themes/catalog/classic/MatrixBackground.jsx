...
export default function MatrixBackground({ manifest }) {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      // Heavier fade trail — keeps the rain subtle so UI text stays readable
      ...
      for (let i = 0; i < columns.length; i++) {
        ...
        // Shorter trail — fewer chars per column
        for (let j = 0; j < 10; j++) {
          ...
          if (j === 0) {
            // Head — dimmer than before
            ctx.fillStyle = 'rgba(180, 255, 200, 0.6)';
          } else if (j < 3) {
            ctx.fillStyle = `rgba(34, 197, 94, ${0.4 - j * 0.08})`;
          } else {
            const fade = Math.max(0, 0.25 - j * 0.025);
