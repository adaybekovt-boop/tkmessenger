...
export function useDropSession({ getIdentity, broadcastEphemeral, sendEphemeralTo, getConn } = {}) {
  ...
  // Freeze heartbeat/reconnect during the entire drop handshake + transfer,
  // not just TRANSFERRING. A reconnect during REQUESTING or AWAITING_CONSENT
  // would kill the DataChannel before the file transfer even starts.
  const isDropInProgress =
    state.status === DropStatus.TRANSFERRING ||
    state.status === DropStatus.REQUESTING ||
    state.status === DropStatus.AWAITING_CONSENT;
