...
export class DropManager extends Emitter {
  ...
  constructor({ getIdentity, broadcastEphemeral, sendEphemeralTo, getConn } = {}) {
    ...
    // Auto-return timers (fire on DONE → BEACON) tracked on the instance so
    // dispose() can cancel them — otherwise they'd keep firing state patches
    // on a torn-down manager.
    this._autoReturnTimer = null;
    // Throttling state for onProgressUpdate (moved from a closure so dispose
    // can cancel an in-flight rAF).
    this._lastProgressEmit = 0;
    this._pendingProgressFrame = null;
  ...
  dispose() {
    ...
    if (this._pendingProgressFrame) {
      cancelAnimationFrame(this._pendingProgressFrame);
      this._pendingProgressFrame = null;
    }
    // Drop transfer-engine callbacks to release closures that captured `this`.
    if (this._transferEngine) {
      this._transferEngine.onProgressUpdate = null;
      this._transferEngine.onFileReady = null;
      this._transferEngine.onTransferFailed = null;
    }
    this._publisher.stop();
    this._listener.stop();
    this.clear();
  ...
  _handleDropReq(remoteId, packet) {
    ...
    // Reject malformed / hostile requests before they ever reach the consent
    // prompt. Caps: max 100 files, 5 GB total, individual entries must be
    // objects with sane name/size. A malicious peer could otherwise send
    // files: Array(1e6) or totalSize: Infinity and exhaust memory/UI.
    const MAX_FILES = 100;
    const MAX_TOTAL_BYTES = 5_000_000_000;
    const MAX_FILE_BYTES = 2_000_000_000;

    const rawFiles = Array.isArray(packet.files) ? packet.files : null;
    if (!rawFiles || rawFiles.length === 0 || rawFiles.length > MAX_FILES) {
      this._sendTo(remoteId, { type: 'drop-rej', reason: 'invalid' });
      return;
    }
    ...
    for (const f of rawFiles) {
      ...
      files.push({
        name: String(f.name || 'file').slice(0, 256),
        size,
    ...
    const totalSize = Number(packet.totalSize);
    if (!Number.isFinite(totalSize) || totalSize < 0 || totalSize > MAX_TOTAL_BYTES) {
      this._sendTo(remoteId, { type: 'drop-rej', reason: 'too_large' });
      return;
    }

    const quality = ['high', 'fast', 'original'].includes(packet.quality)
      ? packet.quality
      : 'high';
  ...
  async _startFileSend(remotePeerId, quality) {
    ...
    try {
      ...
      // Auto-return to BEACON after 3s (tracked so dispose can cancel).
      this._scheduleAutoReturn(3000);
    } catch (err) {
      if (this._state.status === DropStatus.TRANSFERRING) {
        this._transitionTo(DropStatus.ERROR, {
          error: err,
  ...
  _setupTransferCallbacks() {
    ...
    // State lives on the instance (this._lastProgressEmit / this._pendingProgressFrame)
    // so dispose() can cancel any in-flight rAF.
    this._transferEngine.onProgressUpdate = (msgId, percent, statusText) => {
      ...
      if (percent === 0 || percent >= 100 || now - this._lastProgressEmit >= 66) {
      ...
      } else if (!this._pendingProgressFrame) {
        // Schedule a deferred update so mobile UI still catches up
        this._pendingProgressFrame = requestAnimationFrame(() => {
          this._pendingProgressFrame = null;
          if (this._disposed) return;
    ...
    this._transferEngine.onFileReady = (msgId, fileUrl, metadata) => {
      ...
      // Auto-return to BEACON after 10s (tracked so dispose can cancel).
      this._scheduleAutoReturn(10000);
    };
