...
import { getCachedBundle } from '../core/bundleCache.js';
...
export function useConnections(
  ...
  const attachConn = useCallback((conn, channel) => {
    ...
    const onOpen = () => {
      ...
      if (ch === 'reliable') {
        ...
        void (async () => {
          ...
          // Pull the peer's prekey bundle on first contact so a future X3DH
          // session can skip the interactive exchange. Skipped if cached.
          try {
            const cached = await getCachedBundle(remoteId);
            if (!cached) {
              void h.sendEncrypted(remoteId, { type: 'bundle_req', nonce: Date.now() });
            }
          } catch (_) {}
        })();
      } else {
    ...
    const routerCtx = {
      ...
      onGameMessage: (rid, payload) => h.onGameMessage?.(rid, payload),
    };
