...
export function usePeerRegistry() {
  ...
  // Hydrate profile displayNames + avatars from IndexedDB on mount. Walks the
  // peers store (which has the saved displayName) and avatars store (stored
  // separately because ~100KB each would blow the 5MB localStorage quota).
  // This is what lets the contact list show names instead of ORBIT-XXXXXX after
  // a page reload, even if the localStorage profile cache was wiped.
  useEffect(() => {
    let active = true;
    (async () => {
      const stored = safeJsonParse(localStorage.getItem(STORAGE.profiles), {}) || {};
      ...
      let idbPeers = [];
      try {
        idbPeers = await getAllPeers();
      } catch (_) {}
      for (const rec of idbPeers) {
        if (rec?.id) ids.add(rec.id);
      }
      const peerById = new Map(idbPeers.map((p) => [p.id, p]));
      for (const pid of ids) {
        if (!active) break;
        const lsEntry = stored[pid] || {};
        const idbEntry = peerById.get(pid) || {};
        const displayName = lsEntry.displayName || idbEntry.displayName || '';
        let avatarDataUrl = typeof lsEntry.avatarDataUrl === 'string' ? lsEntry.avatarDataUrl : null;
        try {
          const av = await getAvatar(pid);
          if (av) avatarDataUrl = av;
        } catch (_) {}
        if (!active) break;
        if (!displayName && !avatarDataUrl) continue;
        setProfilesByPeer((prev) => {
          ...
          return {
            ...
            [pid]: {
              ...
              avatarDataUrl: avatarDataUrl || existing.avatarDataUrl || null
            }
          };
        });
      }
      ...
      try {
        ...
        if (Array.isArray(idbPeers) && idbPeers.length) {
          savedPeers = idbPeers
            ...
            .filter((p) => isTrustedPeerId(p.id));
          setPeers(savedPeers.map((p) => ({
            id: p.id, status: 'offline', lastSeenAt: p.lastSeenAt, displayName: p.displayName
          })));
