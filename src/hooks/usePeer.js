...
export function usePeer({ enabled = true, desiredPeerId = '', localProfile = null } = {}) {
  ...
  const attachConn = useCallback((conn) => {
    ...
    const onOpen = () => {
      ...
      if (!selectedPeerId) setSelectedPeerId(remoteId);
    ...
    const onData = (data) => {
      ...
      pushMessage(remoteId, { id: `${remoteId}:${ts}:${Math.random().toString(16).slice(2)}`, from: remoteId, to: peerId, text, ts, delivery: 'received' });
    };
    ...
    conn.on('data', onData);
  }, [flushOutboxForPeer, localProfile, peerId, pushMessage, selectedPeerId, upsertPeer]);
  ...
  const sendText = useCallback((targetId, text) => {
    ...
    const conn = connsRef.current.get(normalized);
    const ts = now();
    const msgId = `${peerId}:${ts}:${Math.random().toString(16).slice(2)}`;
    const msg = { id: msgId, from: peerId, to: normalized, text: body, ts, delivery: conn?.open ? 'sent' : 'queued' };
    ...
    try {
      conn.send({ type: 'text', text: body, ts, from: peerId });
  ...
  }, [peerId, pushMessage, queueOutbox, updateMessage]);
