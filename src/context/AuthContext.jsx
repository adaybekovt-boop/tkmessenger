...
export function AuthProvider({ children }) {
  ...
  const completeOnboarding = useCallback(async ({ displayName, password, confirm, avatarDataUrl = null }) => {
    ...
    // N=2^17 (131072) — 2026-grade scrypt hardness. Verify uses N from the
    // stored record, so existing profiles keep working with their old params.
    const passRecord = await deriveScryptRecord({ username: name, password, params: { N: 131072, r: 8, p: 1, dkLen: 32 } });
    // Derive a KEK right now so the freshly-onboarded session can start
    // wrapping ratchet state immediately. `verifyScryptRecordEx` re-runs
    // scrypt, which costs but keeps onboarding and unlock on the same path.
    try {
      const { dkBytes } = await verifyScryptRecordEx({ username: name, password, record: passRecord });
      if (dkBytes) await setVaultKek(dkBytes);
    } catch (_) {}
    ...
    if (locked.passRecord && locked.passRecord.v !== 2) {
      try {
        const fresh = await deriveScryptRecord({
          ...
          password,
          params: {
            N: Number(locked.passRecord.N) || 131072,
        ...
        localStorage.setItem(STORAGE.profile, JSON.stringify({ ...existing, passRecord: fresh }));
      } catch (_) {
