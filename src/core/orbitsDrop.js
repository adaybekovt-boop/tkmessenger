...
export class OrbitsDrop {
   ...
   *
   * Safety nets (all resolve the promise so sendFile() detects the dead
   * channel on its next `conn.send`, rather than hanging forever):
   *   - `close`/`error` on the DataChannel — bufferedamountlow will never fire.
   *   - 10 s timeout — some browsers have flaky bufferedamountlow under load.
   */
  _waitForDrain(dc) {
    ...
    return new Promise((resolve) => {
      ...
      const cleanup = () => {
        ...
        try { dc.removeEventListener('close', onDead); } catch (_) {}
        try { dc.removeEventListener('error', onDead); } catch (_) {}
      };
      ...
      const timeoutId = setTimeout(() => { cleanup(); resolve(); }, 10_000);
    });
  }
