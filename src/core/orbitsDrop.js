...
export class OrbitsDrop {
  ...
  async sendFile(file, conn, statusMsgId, fileId = crypto.randomUUID()) {
    return new Promise((resolve, reject) => {
      ...
      // Get the underlying RTCDataChannel for precise buffer control
      const dataChannel = conn.dataChannel;
      if (!dataChannel) {
        return reject(new Error('RTCDataChannel not accessible'));
      }
      ...
      const sendNextChunk = () => {
        ...
        if (dataChannel.bufferedAmount > this.MAX_BUFFER_SIZE) {
          // Temporarily attach listener to resume when buffer clears
      ...
      // Ensure dataChannel is configured with a threshold for our backpressure
