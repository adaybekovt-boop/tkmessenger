...
// ── Security: все CryptoKey создаются с extractable=false ──
// Ключи живут только внутри замыкания, недоступны через консоль/глобалы.
...
// Non-exportable AES-256-GCM key (extractable: false)
...
async function deriveLocalKey({ password, nickname, salt, saltB64, iterations, version }) {
  ...
  // Default fallback by version — callers that store data MUST persist the
  // iteration count alongside the ciphertext so future defaults don't break
  // old records. v3 default meets OWASP 2026 PBKDF2-SHA256 guidance (600k).
