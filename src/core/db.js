...
// v9 — file_blobs store for chat attachments (images, videos, documents).
// Structure mirrors voice_blobs: { id, blob, mime, name, size, thumb, createdAt }.
const DB_VERSION = 9;
...
export function openDatabase() {
  ...
  dbPromise = openDB(DB_NAME, DB_VERSION, {
    upgrade: async (db, oldVersion, _newVersion, transaction) => {
      ...
      // v7 — Double Ratchet state (per peer, structured-cloned).
      if (!db.objectStoreNames.contains('ratchet_state')) {
        db.createObjectStore('ratchet_state', { keyPath: 'peerId' });
      }

      // v8 — X3DH prekeys. `kind` = 'spk' | 'opk'; `used` marks one-time
      // prekeys that have been consumed (retained briefly for late inbound).
      if (!db.objectStoreNames.contains('prekeys')) {
        const pk = db.createObjectStore('prekeys', { keyPath: 'id' });
        pk.createIndex('kind', 'kind', { unique: false });
        pk.createIndex('used', 'used', { unique: false });
        pk.createIndex('kind_used', ['kind', 'used'], { unique: false });
      }
      // v9 — file attachments (images / videos / documents) sent in chats.
      // Kept separate from `messages` so the row stays small (metadata only)
      // and so we can clear attachments independently from text history.
      if (!db.objectStoreNames.contains('file_blobs')) {
        db.createObjectStore('file_blobs', { keyPath: 'id' });
      }

      if (oldVersion < 7 && db.objectStoreNames.contains('session_keys')) {
        // Wipe legacy static-key sessions — Phase 2 breaks compat on purpose.
        try {
          const legacy = transaction.objectStore('session_keys');
          let cur = await legacy.openCursor();
          while (cur) {
            await cur.delete();
...
// ─── Double Ratchet state (v7) ───────────────────────────────

export async function saveRatchetState(snapshot) {
  if (!snapshot || !snapshot.peerId) return false;
  const db = await openDatabase();
  await db.put('ratchet_state', snapshot);
  return true;
}

export async function loadRatchetState(peerId) {
  const db = await openDatabase();
  return (await db.get('ratchet_state', String(peerId || ''))) || null;
}

export async function deleteRatchetState(peerId) {
  const db = await openDatabase();
  await db.delete('ratchet_state', String(peerId || ''));
  return true;
}

export async function clearAllRatchetState() {
  const db = await openDatabase();
  await db.clear('ratchet_state');
  return true;
}
 ...
 */
export async function saveFileBlob(id, blob, meta = {}) {
  const db = await openDatabase();
...
export async function getFileBlob(id) {
  const db = await openDatabase();
  return (await db.get('file_blobs', String(id))) || null;
}
export async function deleteFileBlob(id) {
  const db = await openDatabase();
  await db.delete('file_blobs', String(id));
  return true;
}
...
export async function savePeer(peer) {
  ...
  if (!id) return false;
  const existing = (await db.get('peers', id)) || {};
  const trusted = peer?.trusted === true || existing.trusted === true;
  // Merge with existing so partial updates (e.g. just a lastSeenAt bump) don't
  // wipe a previously-saved displayName or pubKey.
  const nextDisplayName = typeof peer.displayName === 'string' && peer.displayName
    ? peer.displayName.slice(0, 64)
    : (existing.displayName || '');
  await db.put('peers', {
    id,
    displayName: nextDisplayName,
    lastSeenAt: Number(peer.lastSeenAt || existing.lastSeenAt || 0) || 0,
    trusted,
    pubKey: peer.pubKey || existing.pubKey || null,
    trustLevel: peer.trustLevel || existing.trustLevel || (trusted ? 1 : 0),
    addedAt: existing.addedAt || peer.addedAt || Date.now()
  });
  return true;
}
...
export async function deleteAvatar(peerId) {
  const db = await openDatabase();
  await db.delete('avatars', String(peerId || ''));
  return true;
}
