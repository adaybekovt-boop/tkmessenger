...
export function isWireReady(peerId) {
  return isReady(peerId);
}

/**
 * Development assertion: warns if someone tries to send a non-ciphertext,
 * non-handshake payload directly on a connection.
 */
export function assertEncryptedOrHandshake(data) {
  if (typeof data === 'string' && isWireCiphertext(data)) return;
  if (data && typeof data === 'object') {
    const t = data.type;
    if (t === 'wireHello' || t === 'wireRekey') return;
  }
  if (import.meta.env?.DEV) {
    console.error('[wireCrypto] Unencrypted payload detected:', data?.type || typeof data);
  }
}
