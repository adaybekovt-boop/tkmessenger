      ...
      try {
        ...
        if (!store.indexNames.contains('byChatTime')) {
          ...
          req2.onupgradeneeded = (ev2) => {
            const store2 = ev2.target.transaction.objectStore('messages');
            if (!store2.indexNames.contains('byChatTime')) {
              try { store2.createIndex('byChatTime', ['chatId', 'ts'], { unique: false }); } catch (_) {}
            }
          };
  ...
  return new Promise((resolve, reject) => {
    ...
    const range =
      beforeTs != null
        ...
        : IDBKeyRange.lowerBound([chatId, 0]);
    const req = idx.openCursor(range, 'prev');
    req.onsuccess = (ev) => {
      const cursor = ev.target.result;
      if (cursor && out.length < cap) {
        if (cursor.key[0] !== chatId) {
            // Skip other chats if we are scanning backwards from the end of the DB
            cursor.continue([chatId, Date.now() + 100000000]);
            return;
        }
...
function updateStatusByChatTs(chatId, ts, status) {
  return new Promise((resolve, reject) => {
    ...
    if (store.indexNames.contains('byChatTime')) {
      const idx = store.index('byChatTime');
      const req = idx.openCursor(IDBKeyRange.only([chatId, ts]));
      req.onsuccess = (ev) => {
        const cursor = ev.target.result;
        if (cursor) {
