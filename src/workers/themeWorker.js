...
function initSakura() {
  const n = Math.min(80, Math.max(20, params.sakuraCount | 0));
  sakuraPetals = [];
  for (let i = 0; i < n; i++) {
    sakuraPetals.push({
      x: Math.random() * width,
...
function drawSakuraFrame(t) {
  ...
  for (const p of sakuraPetals) {
    ...
    g.addColorStop(0, 'rgba(255, 218, 230, 0.95)');
    g.addColorStop(0.45, 'rgba(255, 150, 180, 0.65)');
