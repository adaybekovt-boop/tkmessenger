...
/** OPTIMIZED: Parallel decryption for batch operations */
async function decryptBatch(encList) {
  if (!vaultKey) throw new Error('Vault is locked');
  return Promise.all(
    encList.map(async (enc) => {
      try {
        if (!enc) return null;
        return await decryptFromVault(enc);
      } catch (err) {
        console.error('Batch decrypt error:', err);
        return null;
      }
    })
  );
...
self.onmessage = async (e) => {
  ...
  try {
    ...
    switch (type) {
      case 'derive':
        vaultKey = await deriveVaultKey(payload.password, payload.nickname);
        result = { ok: true };
        break;
      case 'lock':
        vaultKey = null;
      ...
      case 'decrypt':
        ...
        break;
      case 'decryptBatch':
        result = await decryptBatch(payload.encList);
        break;
      case 'sha256Hex':
